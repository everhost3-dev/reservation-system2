...
/**
 * Schedules a browser notification to be displayed at a specific time.
 * @param title The title of the notification.
 * @param options The notification options (e.g., body, icon).
 * @param scheduledTime The Date object representing when the notification should appear.
 */
export const scheduleNotification = (title: string, options: NotificationOptions, scheduledTime: Date) => {
    ...
    if (delay > 0) {
        setTimeout(() => {
            // Re-check permission in case it was revoked while waiting
            if (Notification.permission === 'granted') {
                new Notification(title, options);
            }
        }, delay);
    } else {
        console.warn('Attempted to schedule a notification for a time in the past.');
    }
};
