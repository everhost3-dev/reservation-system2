...
const AdminDashboardView: React.FC<AdminDashboardViewProps> = ({ onLogout }) => {
    ...
    const seatLocations = useMemo(() => LOCATIONS.filter(l => l !== '스터디룸'), []);
    const [viewingLocation, setViewingLocation] = useState(seatLocations[0]);
    const [viewingTimeSlot, setViewingTimeSlot] = useState(TIME_SLOTS.find(ts => ts.id === 'study1')?.id || TIME_SLOTS[0].id);
    ...
    const todayStudyRecords = useMemo(() => {
        const todayStr = new Date().toISOString().split('T')[0];
        return studyRecords.filter(r => r.date === todayStr);
    }, [studyRecords]);
    ...
    return (
        <div className="container mx-auto px-4 py-8 max-w-7xl">
            ...
            <div className="p-6 rounded-lg bg-white dark:bg-gray-800 shadow mb-8">
                <h2 className="text-xl font-bold mb-4">실시간 좌석 현황</h2>
                <div className="flex flex-col sm:flex-row gap-4 mb-4">
                    <div className="flex-1">
                        <label htmlFor="location-select" className="block text-sm font-medium mb-1">학습실 선택</label>
                        <select
                            id="location-select"
                        ...
                        >
                            {seatLocations.map(loc => <option key={loc} value={loc}>{loc}</option>)}
                        </select>
                    </div>
                    <div className="flex-1">
                        <label htmlFor="timeslot-select" className="block text-sm font-medium mb-1">시간대 선택</label>
                        <select
                            id="timeslot-select"
                            ...
                            className="w-full p-2 border rounded-lg bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600"
                        >
                            {TIME_SLOTS.map(slot => <option key={slot.id} value={slot.id}>{slot.label} ({slot.time})</option>)}
                        </select>
                    </div>
                </div>
                <SeatLayout
                    location={viewingLocation}
                    selectedTimeSlot={viewingTimeSlot}
                    reservations={todayStudyRecords}
                    readOnly={true}
